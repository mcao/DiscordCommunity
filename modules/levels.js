    ...
    bot.register('leaderboard', msg => {
        ...
        reactionButtons: [{
            ...
            response: msg => {
                ...
                msg.getReaction('◀').then(users => {
                    users.forEach(v => {
                        if (v.id !== bot.user.id) msg.removeReaction('◀', v.id);
                    });
                });
                ...
                while (start < end && userIDs[start]) {
                    if (bot.users.get(userIDs[start])) var user = bot.users.get(userIDs[start]).username;
                    else user = 'Unknown';
    ...
    bot.register('rank', (msg, args) => {
        if (msg.mentions[0] && (msg.mentions[0].username || args.length === 18 || args.length === 17)) {
            ...
            if (bot.profiles[msg.mentions[0].id] || bot.profiles[args]) {
                ...
                if (bot.profiles[id].lastRankAssignment - 1 > -1) var rank = msg.channel.guild.roles.get(ranks[bot.profiles[id].lastRankAssignment - 1].id).name;
                else rank = 'None';
            ...
            if (bot.profiles[msg.author.id]) {
                if (bot.profiles[msg.author.id].lastRankAssignment - 1 > -1) rank = msg.channel.guild.roles.get(ranks[bot.profiles[msg.author.id].lastRankAssignment - 1].id).name;
                else rank = 'None';

                let user2 = msg.author;
                bot.createMessage(msg.channel.id, {
                    embed: {
                        title: `${user2.username}#${user2.discriminator}`,
                        author: {
                            name: 'Discord Community',
                            icon_url: 'https://cdn.discordapp.com/avatars/392450607983755264/071e72220fae40698098221d52df3e5f.jpg?size=256',
                        },
                        ...
                        fields: [{
                            name: 'Message count',
                            value: `${bot.profiles[user2.id].messageCount}`,
                            inline: true,
                        },
                        {
                            name: 'Highest rank',
                        ...
                        thumbnail: {
                            url: user2.avatarURL.replace('?size=128', ''),
                        },
                    },
                });
            } else {
